...
class RAGService:
    ...
    def fetch_airport_context(self, icao: str):
        ...
        try:
            ...
            if channels:
                ...
                if freqs:
                    context_parts.append(f"FREQUENCIES AT THIS AIRPORT: {', '.join(set(freqs))}")
    ...
    def get_context(self, text_hint: str = "", mount: str = None) -> str:
        ...
        if mount:
            ...
            if airport_ctx:
                context_parts.append(f"### DYNAMIC AIRPORT CONTEXT ({icao})\n{airport_ctx}")

        # Static Rules
        context_parts.append("### GENERAL ATC RULES")
        if "runway_rules" in self.data:
            context_parts.append(f"RUNWAY RULES: {self.data['runway_rules']['description']}")
            for p in self.data['runway_rules']['patterns']:
                context_parts.append(f"- {p['instruction']}")

        if "tail_number_rules" in self.data:
            context_parts.append(f"AIRCRAFT ID RULES: {self.data['tail_number_rules']['description']}")
        ...
        if "waypoint_rules" in self.data:
            ...
            is_waypoint_related = any(s.lower() in text_hint.lower() for s in synonyms) if text_hint else True
            if is_waypoint_related:
                context_parts.append(f"WAYPOINT/REPORTING RULES: {self.data['waypoint_rules']['description']}")
        ...
        if "number_pronunciation_variations" in self.data:
            vars_str = ", ".join([f"{k}: {v}" for k, v in self.data['number_pronunciation_variations'].items()])
            context_parts.append(f"PRONUNCIATION VARIATIONS (Recognize these as numbers): {vars_str}")

        if "standard_output_mapping" in self.data:
            mapping_str = ", ".join([f"{k} -> {v}" for k, v in self.data['standard_output_mapping'].items()])
