...
class GeminiTranscriber:
            ...
            for frame in container.decode(audio=0):
                ...
                for resampled_frame in resampled_frames:
                    ...
                    if frame_count % 100 == 0:
                         ...
                         print(f"  [STREAM] {int(elapsed_sec)}s captured | Queue: {qsize} frames")
    ...
    async def transcribe_gen(self, url):
        ...
        import asyncio
        if not self.api_key:
            yield {"text": "[Error: GEMINI_API_KEY not configured]", "speaker": "UNKNOWN"}
            return
            ...
            try:
                ...
                if response.text:
                    try:
                        ...
                        if result.text.strip() and result.text.upper() != "UNINTELLIGIBLE STATIC":
                    ...
                    while len(buffer) >= frame_size_bytes:
                        ...
                        if is_speech:
                            ...
                            if is_speech_active:
                                speech_buffer += frame
                                silence_frames += 1
                                if silence_frames >= min_silence_frames_to_stop or len(speech_buffer) >= max_speech_bytes:
                                    # End of utterance
                                    if len(speech_buffer) > sample_rate * 2 * 0.5:
                                        duration = len(speech_buffer) / (sample_rate * 2)
                                        print(f"  <<< [SPEECH ENDED] Transcribing {duration:.1f}s audio...")
